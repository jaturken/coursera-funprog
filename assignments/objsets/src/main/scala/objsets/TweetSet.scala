...
abstract class TweetSet {
  ...
  def ascendingByRetweet: Trending = 
    if (this.isEmpty) new EmptyTrending
      ...
      val newt = this.remove(min).ascendingByRetweet
      new NonEmptyTrending(min, newt)
    }
...
class Empty extends TweetSet {
  ...
  override def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = accu
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet =
    ...
    else if (that.contains(head)) tail.union(that)
    else  tail.union(that.incl(head))
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter(tw => containsWord(tw,google))

  val appleTweets: TweetSet = TweetReader.allTweets.filter(tw => containsWord(tw,apple))
  ...
  def containsWord(tw: Tweet, keywors: List[java.lang.String]): Boolean =
    if (keywors.isEmpty) false
    else if (tw.text.contains(keywors.head)) true
    else containsWord(tw,keywors.tail)
}
